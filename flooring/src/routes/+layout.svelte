...
<main class="dark:text-white bg-gray-100">
	<div id='nav' class="fixed top-0 w-screen h-[4.5rem] z-[999] overflow-x-clip border-b-gray-300 border-b duration-200 hidden md:block -translate-y-full" in:fly={{ y: -100, duration: 400 }} out:fly={{ y: -100, duration: 400 }}>
		<AppBar gridColumns="grid-cols-3"  class="w-screen flex-col flex h-full justify-center text-white border-b-white" slotDefault="flex justify-center place-items-center" background="dark:bg-black bg-gray-200" slotTrail="flex justify-end">
